...
- 2026-04-23: MentalManip著者修正（Xu et al. → Wang et al.）。References追加（Wang et al. 2024）。Arai出版社修正（Toyo Keizai → Shin-yo-sha・Rev. ed.追加）。6.1に221件除外根拠追加（JS描画失敗3件）。
- 2026-04-21: Section 2.8末尾にエコーチェンバー・スコープ明示文追加（Sunstein, 2017）。References追加（Sunstein 2017）。Limitation 5.9に与沢本7章分析結果追記（book-format corpus cross-genre replication）。.gitignoreタイポ修正（personalmomo→personalmemo）。
